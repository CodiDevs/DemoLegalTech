...
1. **Design Thinking**: Before coding, understand the context and commit to a BOLD aesthetic direction:
...
3. **Typography**: Choose fonts that are beautiful and interesting (e.g. avoid Arial, use distinctive display fonts paired with refined body fonts).
4. **Color & Theme**: Commit to a cohesive aesthetic with CSS variables.
5. **Motion**: Use animations for effects and high-impact moments.
6. **Spatial Composition**: Use unexpected layouts, asymmetry, or generous negative space.
7. **Backgrounds**: Create atmosphere and depth (gradients, noise, etc.).
...
Work in two passes:
1. Brainstorm a short design plan based on the brief: create a compact token system with color (4-6 hex values), type (2+ roles), layout, and signature element.
2. Review that plan against the brief before building. If it reads like a generic default, revise it. Only after confirming its relative uniqueness should you start writing code.
